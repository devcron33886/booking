    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookings.update", [$booking->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('staff') ? 'is-invalid' : '' }}" name="staff_id" id="staff_id">
                    @foreach($staff as $id => $entry)
                        <option value="{{ $id }}" {{ (old('staff_id') ? old('staff_id') : $booking->staff->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('staff'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('meeting_time') ? 'is-invalid' : '' }}" type="text" name="meeting_time" id="meeting_time" value="{{ old('meeting_time', $booking->meeting_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $booking->phone_number) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{{ old('notes', $booking->notes) }}</textarea>
                @if($errors->has('notes'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('privacy') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="privacy" id="privacy" value="1" {{ $booking->privacy || old('privacy', 0) === 1 ? 'checked' : '' }} required>
