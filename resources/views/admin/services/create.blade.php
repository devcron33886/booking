    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.services.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('currency') ? 'is-invalid' : '' }}" name="currency" id="currency">
                    ...
                    @foreach(App\Models\Service::CURRENCY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('currency', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('service_description') ? 'is-invalid' : '' }}" name="service_description" id="service_description" required>{{ old('service_description') }}</textarea>
